...
const Box = ({ position, rotation }: { position: number[], rotation: number[] }) => {
...
const AnimatedBoxes = () => {
    ...
    useFrame((state, delta) => {
        if (groupRef.current) {
            (groupRef.current as any).rotation.x += delta * 0.05;
        }
    });
...
export const Scene = () => {
    ...
    return (
        <div className="w-full h-full z-0">
            <Canvas camera={{ position: [cameraPosition[0], cameraPosition[1], cameraPosition[2]] as [number, number, number], fov: 40 }}>
